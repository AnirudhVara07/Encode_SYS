...
def rsi_cross_trend_filter_signals(
    *,
    ...
    # Entry: RSI crosses below/into oversold region AND we are in an uptrend (price above EMA).
    crossed_into = rsi_prev >= rsi_lower and rsi_now < rsi_lower
    uptrend = close_now > ema_now
    enter_long = crossed_into and uptrend

    # Exit: RSI crosses up above upper threshold.
