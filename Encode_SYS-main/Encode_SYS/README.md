...
- `GET /report` — Realized P&amp;L stats (FIFO on BTC sells), autonomous trade list, recent blocked; when **autonomous is off** and a **strategy profile** exists, calls OpenAI for 2–3 improvement suggestions (requires `OPENAI_API_KEY`).
...
Copy [`backend/.env.example`](backend/.env.example) and export variables (no secrets in the repo).
